...
const logActivity = async (userId, type, action, ip) => {
  (await pool.query("INSERT INTO activity (user_id, type, action, ip) VALUES (?, ?, ?, ?)", [userId, type, action, ip || '']))[0];
};
...
const auth = async (req, res, next) => {
  ...
  try {
    ...
    // Check ban
    const ban = await getActiveBan(decoded.id, ip);
    if (ban) {
    ...
    const user = (await pool.query("SELECT * FROM users WHERE id = ? AND is_active = 1", [decoded.id]))[0][0];
...
app.post('/api/login', async (req, res) => {
  ...
  // Check IP ban
  const ipBan = await getActiveBan(null, ip);
  if (ipBan) {
    return res.status(403).json({
  ...
  // Find user
  const user = (await pool.query("SELECT * FROM users WHERE phone = ? AND password = ? AND is_active = 1", [normPhone, password]))[0][0];
  if (!user) {
    return res.status(401).json({ error: 'Hatalı telefon veya şifre!' });
  }

  // Check user-specific ban
  const userBan = await getActiveBan(user.id, null);
  if (userBan) {
    return res.status(403).json({
  ...
  if (user.current_ip && user.current_ip !== ip && user.role !== 'admin') {
    ...
    if (newCount >= 2) {
      // 2. DENEME → HESAP SİL
      (await pool.query("UPDATE users SET is_active = 0, multi_login_count = ? WHERE id = ?", [newCount, user.id]))[0];
      await logActivity(user.id, 'ban_permanent', `Hesap kapatıldı! Multi-login #${newCount}. Yeni IP: ${ip}, Eski IP: ${user.current_ip}`, ip);
    ...
    await pool.query("INSERT INTO bans (user_id, ip, reason, expires_at) VALUES (?, ?, ?, ?)", [user.id, ip, banReason, expiresAt]);
    await pool.query("INSERT INTO bans (user_id, ip, reason, expires_at) VALUES (?, ?, ?, ?)", [user.id, user.current_ip, banReason, expiresAt]);

    (await pool.query("UPDATE users SET current_ip = ?, multi_login_count = ? WHERE id = ?", [ip, newCount, user.id]))[0];
    await logActivity(user.id, 'ban_temp', `30dk ban. Eski IP: ${user.current_ip}, Yeni IP: ${ip}`, ip);
  ...
  const now = new Date().toISOString();
  (await pool.query("UPDATE users SET current_ip = ?, last_login = ? WHERE id = ?", [ip, now, user.id]))[0];
  await logActivity(user.id, 'login', 'Giriş yapıldı', ip);
...
app.get('/api/users', auth, adminOnly, async (req, res) => {
  const users = (await pool.query("SELECT id, phone, password, role, is_active, current_ip, multi_login_count, created_at, last_login FROM users ORDER BY id DESC", []))[0];
  res.json(users);
});
...
app.post('/api/users', auth, adminOnly, async (req, res) => {
  ...
  try {
    const result = (await pool.query("INSERT INTO users (phone, password, role) VALUES (?, ?, ?)", [normPhone, password, role || 'user']))[0];
...
app.delete('/api/users/:id', auth, adminOnly, async (req, res) => {
  const { id } = req.params;
  (await pool.query("DELETE FROM users WHERE id = ? AND role != 'admin'", [id]))[0];
  res.json({ success: true });
});

// Reactivate user (unban)
app.post('/api/users/:id/reactivate', auth, adminOnly, async (req, res) => {
  const { id } = req.params;
  (await pool.query("UPDATE users SET is_active = 1, multi_login_count = 0 WHERE id = ?", [id]))[0];
  (await pool.query("DELETE FROM bans WHERE user_id = ?", [id]))[0];
  res.json({ success: true });
});
...
app.get('/api/users/:id/stats', auth, adminOnly, async (req, res) => {
  const { id } = req.params;
...
app.post('/api/activity', auth, async (req, res) => {
  const { page } = req.body;
  await logActivity(req.user.id, 'page', page, req.userIP);
  res.json({ success: true });
});
  ...
  for (let i = 0; i < daysBack; i++) {
    ...
    try {
      ...
      for (const m of data.data) {
        ...
        const exists = (await pool.query('SELECT id FROM gol6_matches WHERE match_id = ?', [m.MatchID]))[0][0];
        if (exists) continue;
        ...
        await pool.query(`INSERT INTO gol6_matches (match_id, match_date, team1, team2, league, league_flag, home_score, away_score, total_goals, home_win, draw, away_win, over25, under25, iy_15_ust, iy_kg_var, bets_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
app.post('/api/nosy/gol6/sync', auth, async (req, res) => {
  try {
    ...
    const total = (await pool.query('SELECT COUNT(*) as cnt FROM gol6_matches', []))[0][0].cnt;
...
app.get('/api/nosy/gol6/similar/:matchId', auth, async (req, res) => {
  try {
    ...
    const ref = (await pool.query('SELECT * FROM gol6_matches WHERE match_id = ?', [matchId]))[0][0];
...
const upsertPlayerSQL = `
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
    ...
    ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
  ON DUPLICATE KEY UPDATE 
    name=VALUES(name), common_name=VALUES(common_name, team_id=VALUES(team_id, team_name=VALUES(team_name,
    league_id=VALUES(league_id, league_name=VALUES(league_name, season_id=VALUES(season_id,
    appearances=VALUES(appearances, minutes=VALUES(minutes, goals=VALUES(goals, assists=VALUES(assists,
    yellowcards=VALUES(yellowcards, redcards=VALUES(redcards, shots_total=VALUES(shots_total,
    shots_on_target=VALUES(shots_on_target, tackles=VALUES(tackles, saves=VALUES(saves, fouls=VALUES(fouls,
    goals_per_game=VALUES(goals_per_game, assists_per_game=VALUES(assists_per_game, cards_per_game=VALUES(cards_per_game,
    shots_per_game=VALUES(shots_per_game, shots_on_target_pg=VALUES(shots_on_target_pg,
    tackles_per_game=VALUES(tackles_per_game, saves_per_game=VALUES(saves_per_game), synced_at=CURRENT_TIMESTAMP
`;
    ...
    for (const team of teamsData.data) {
      try {
        ...
        for (const entry of squadData.data) {
          ...
          await pool.query(upsertPlayerSQL, [
...
async function syncTeamForm() {
  ...
  const query = `INSERT INTO sm_team_form (team_id, team_name, league_id, match_date, opponent, is_home, goals_for, goals_against, kg_var, over25, over35) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE team_name=VALUES(team_name), is_home=VALUES(is_home), goals_for=VALUES(goals_for), goals_against=VALUES(goals_against), kg_var=VALUES(kg_var), over25=VALUES(over25), over35=VALUES(over35), synced_at=CURRENT_TIMESTAMP`;
  ...
  for (const league of SM_LEAGUES) {
    try {
      ...
      for (const fix of data.data) {
          ...
          // Ev sahibi kaydı
          await pool.query(query, [home.id, home.name, league.id, matchDate, away.name, 1, hg, ag, kgVar, over25, over35]);
          // Deplasman kaydı
          await pool.query(query, [away.id, away.name, league.id, matchDate, home.name, 0, ag, hg, kgVar, over25, over35]);
          totalMatches++;
      }
...
app.get('/api/sportmonks/team-form', auth, async (req, res) => {
  try {
    ...
    const allTeams = (await pool.query('SELECT DISTINCT team_name, team_id FROM sm_team_form', []))[0];
    let bestMatch = null;
    let bestScore = 0;
    ...
    const rows = (await pool.query('SELECT * FROM sm_team_form WHERE team_id = ? ORDER BY match_date DESC LIMIT 5', [bestMatch.team_id]))[0];
...
app.post('/api/sportmonks/team-form-bulk', auth, async (req, res) => {
  try {
    ...
    for (const team of teams) {
      ...
      if (bestMatch) {
        result[team] = (await pool.query('SELECT kg_var, over25, over35 FROM sm_team_form WHERE team_id = ? ORDER BY match_date DESC LIMIT 5', [bestMatch.team_id]))[0];
      }
    }
...
app.get('/api/sportmonks/player-stats', auth, async (req, res) => {
  try {
    ...
    if (player) {
      ...
      rows = (await pool.query(`SELECT * FROM sm_players WHERE 
      ...
      if (rows.length === 0 && clean !== player) {
        const allPlayers = (await pool.query('SELECT * FROM sm_players ORDER BY goals DESC', []))[0];
      ...
      const mapping = (await pool.query('SELECT sm_team_id FROM sm_team_mapping WHERE nosy_name = ?', [team]))[0][0];
      if (mapping) {
        rows = (await pool.query('SELECT * FROM sm_players WHERE team_id = ? ORDER BY goals DESC', [mapping.sm_team_id]))[0];
      } else {
        // Her zaman aksansız eşleştirme yap — en iyi eşleşmeyi bul
        const cleanTeam = removeAccents(team).toLowerCase();
        const allTeams = (await pool.query('SELECT DISTINCT team_name, team_id, league_id FROM sm_players', []))[0];
        ...
        if (bestMatch) {
          rows = (await pool.query('SELECT * FROM sm_players WHERE team_id = ? ORDER BY goals DESC', [bestMatch.team_id]))[0];
          await pool.query('INSERT IGNORE INTO sm_team_mapping (nosy_name, sm_team_id, sm_team_name, league_id) VALUES (?, ?, ?, ?)', [team, bestMatch.team_id, bestMatch.team_name, bestMatch.league_id]);
...
// SM — DB özeti
app.get('/api/sportmonks/status', auth, async (req, res) => {
  const playerCount = (await pool.query('SELECT COUNT(*) as cnt FROM sm_players', []))[0][0].cnt;
  const leagueCount = (await pool.query('SELECT COUNT(DISTINCT league_id) as cnt FROM sm_players', []))[0][0].cnt;
  const mappingCount = (await pool.query('SELECT COUNT(*) as cnt FROM sm_team_mapping', []))[0][0].cnt;
  const lastSync = (await pool.query('SELECT MAX(synced_at) as ts FROM sm_players', []))[0][0].ts;
